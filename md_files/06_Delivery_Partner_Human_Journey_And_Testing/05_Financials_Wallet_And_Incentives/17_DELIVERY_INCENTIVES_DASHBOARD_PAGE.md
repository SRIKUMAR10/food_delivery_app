# 📑 17. Weekly Incentives, Streaks & Gamification Page — Human Journey & Real-Time Testing Blueprint

**Document ID:** `DELIVERY-DOC-17-INCENTIVES`  
**Classification:** Phase 5: Financials, Wallet, Earnings & Incentives  
**Target Screen:** [DeliveryIncentivesDashboardPageUI](file:///lib/features/Delivery Partner Bloc Architecture/Delivery_Incentives Dashboard_page/Delivery_Incentives Dashboard_page_ui.dart)  
**Target BLoC:** `DeliveryIncentivesDashboardPageBloc`  
...
### 🎯 Step Overview
Gamification and milestone incentive tracker for riders. Displays daily target challenges (e.g. 'Complete 12 orders ➔ ₹350 bonus'), weekend peak-hour streaks, monthly tier badges (Silver, Gold, Platinum, Diamond), and regional partner leaderboard.

```mermaid
sequenceDiagram
    autonumber
    actor Rider as 🛵 Delivery Partner
    participant UI as DeliveryIncentivesDashboardPageUI
    participant Bloc as DeliveryIncentivesDashboardPageBloc
    participant Repo as DeliveryPartnerRepository
    participant FS as Cloud Firestore (partner_incentives/{uid})

    Rider->>UI: Interacts with screen
    UI->>Bloc: add(FetchIncentivesEvent())
    Bloc->>Repo: executeOperation()
    Repo->>FS: Real-time query / transaction (computePartnerIncentivesAndBadges)
    FS-->>Repo: Real-time Snapshot
    Repo-->>Bloc: Result Data
    Bloc-->>UI: emit(DeliveryIncentivesLoadedState)
    UI->>Rider: Renders reactive UI update
```

### 🛣️ Navigation Preconditions & Route
- **Route Preceding Screen:** DeliveryEarningsDashboardPageUI
...
- **Target File:** `DeliveryIncentivesDashboardPageUI (lib/features/Delivery Partner Bloc Architecture/Delivery_Incentives Dashboard_page/Delivery_Incentives Dashboard_page_ui.dart)`
- **BLoC State Management:** `DeliveryIncentivesDashboardPageBloc`
...
- **Firestore Document:** `partner_incentives/{uid}`
- **Serverless Cloud Function:** `computePartnerIncentivesAndBadges`
