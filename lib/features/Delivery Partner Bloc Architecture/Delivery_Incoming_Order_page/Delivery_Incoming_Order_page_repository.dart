...
class DeliveryIncomingOrderRepository
  ...
  DeliveryIncomingOrderState _mapRaw(Map<String, dynamic>? raw) {
    ...
    return DeliveryIncomingOrderState(
      ...
      storeLatitude: (raw['storeLatitude'] as num?)?.toDouble() ?? 11.4485,
      storeLongitude: (raw['storeLongitude'] as num?)?.toDouble() ?? 77.6835,
      customerName: raw['customerName'] ?? '',
      customerAddress: raw['customerAddress'] ?? '',
      customerLatitude: (raw['customerLatitude'] as num?)?.toDouble() ?? 11.4580,
      customerLongitude: (raw['customerLongitude'] as num?)?.toDouble() ?? 77.6980,
      driverLatitude: (raw['driverLatitude'] as num?)?.toDouble() ?? 11.4485,
      driverLongitude: (raw['driverLongitude'] as num?)?.toDouble() ?? 77.6835,
