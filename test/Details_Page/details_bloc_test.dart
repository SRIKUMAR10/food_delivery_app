...
void main() {
  group('DetailsBloc', () {
    ...
    test('initial state should be DetailsState with quantity 1', () {
      expect(detailsBloc.state, const DetailsState(quantity: 1));
    });
    ...
    test('DetailsQuantityIncreased should increment quantity by 1', () {
      ...
      expectLater(
        detailsBloc.stream,
        emitsInOrder([const DetailsState(quantity: 2)]),
      );
    });

    test(
      'DetailsQuantityDecreased should decrement quantity by 1 when quantity > 1',
      () async {
        // First increase to 2
        detailsBloc.add(DetailsQuantityIncreased());
