...
void main() {
  ...
  group('DeliveryOrderHistoryPage Localization Tests', () {
    ...
    testWidgets('renders Tamil UI text when locale is Tamil', (tester) async {
      ...
      expect(find.text('அனைத்து நேரம்'), findsOneWidget);
      expect(find.text('இந்த மாதம்'), findsOneWidget);
    ...
    testWidgets('date preset chips are translated in Tamil', (tester) async {
      setDesktopSize(tester);
      when(() => mockBloc.state).thenReturn(taState);
      await tester.pumpWidget(buildPage());
      await tester.pump();
      ...
      expect(find.text('அனைத்து நேரம்'), findsOneWidget);
      expect(find.text('இன்று'), findsOneWidget);
      expect(find.text('நேற்று'), findsOneWidget);
      expect(find.text('இந்த வாரம்'), findsOneWidget);
      expect(find.text('இந்த மாதம்'), findsOneWidget);
      expect(find.text('தனிப்பயன் தேதி'), findsOneWidget);
    });
