...
void main() {
  ...
  group('DeliveryOrderHistoryPage Widget Tests', () {
    ...
    testWidgets('renders the orders table with rows and pagination', (
      ...
      expect(find.text('May 22, 2025 • 10:30'), findsOneWidget);
    ...
    testWidgets('dispatches date preset event from the filter chip', (
      tester,
    ) async {
      setDesktopSize(tester);
      await tester.pumpWidget(buildPage());
      ...
      verify(
        () => mockBloc.add(
          const DeliveryOrderHistoryDatePresetChangedEvent(
            DeliveryOrderHistoryDatePreset.thisMonth,
          ),
        ),
      ).called(1);
    });
