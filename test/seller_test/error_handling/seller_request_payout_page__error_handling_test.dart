...
void main() {
  group('Error Handling UI Tests', () {
    ...
    testWidgets('triggers LoadPayoutDetails event on clicking Retry button', (
      tester,
    ) async {
      when(
        () => bloc.state,
      ).thenReturn(const SellerRequestPayoutError('Network Timed Out'));
      ...
      verify(() => bloc.add(const LoadPayoutDetails())).called(1);
    });
  });
}
